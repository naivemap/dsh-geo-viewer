		...
		async execute(args, exec) {
			...
			if (args.addresses !== void 0) {
				...
				if (ext === "xlsx" || ext === "xls" || ext === "xlsm") {
					const table = await tableToFeatures(await rowsFromXlsx(await ctx.fs.readBytes(target, exec.signal, 67108864)), args, options);
					sourceKind = "xlsx";
					...
					const parsed = await parseTextualSource(await ctx.fs.readText(target, exec.signal), ext, args, options, target.displayPath);
			...
			if (bytes > options.maxBytes) throw new Error(`GeoJSON is ${String(bytes)} bytes, over the ${String(options.maxBytes)}-byte limit - downsample the data (fewer features or properties), or raise the dsh-geo-viewer \`maxBytes\` config`);
